...
Planner's step 2, **"What hardware do I need to serve N users?"**, takes one model (typed in, picked from the catalog, handed over from step 1, or the Calculator's current model), a user count, a context per user, a weight quant/KV dtype, a runtime profile, a minimum per-user decode tok/s and an optional max TTFT, then searches the same GPU-preset × {1, 2, 4, 8} space as *Which hardware fits?* for every candidate that runs at that load, passes the tensor-parallel split check, and meets the tok/s and TTFT targets. A row's own `workload` (context/users) is clamped to that model's `maxPositionEmbeddings` and to `MAX_USERS` before it's calculated — the same clamp the Calculator itself applies on load — so a row's numbers and what **Use** actually loads can never disagree; if the typed context is above the model's max (e.g. left over from a previously picked, longer-context model), a note shows the effective context the rows are sized at. Qualifying rows are ranked by a sort control: **Smallest first** (total VRAM ascending, the default — a personal tool shouldn't bury an already-owned single GPU under a priced cluster just because the cluster has a listed cloud rate) or **Cheapest cloud $/hr** (priced rows by $/hour ascending, then unpriced rows by VRAM). A few close-but-failing rows are shown dimmed with why ("short 6 GB", "14 tok/s < 20", "TP split invalid"). Each qualifying row's **Use** button opens the Calculator with that exact model/hardware/runtime/workload (with speculative decoding off, since Planner rows are never sized with it on). A small strip below shows the smallest qualifying option at 1, 4, 16, 64 and 256 users, for a quick read on how the choice scales. Step 1's **Size hardware** action always wins here: it overrides both an explicit model pick and "use the Calculator's model" for this step, and is still labeled "handed over from step 1". Its inputs are saved in the URL under `ph`-prefixed keys once the panel is touched (the handoff itself rides the same `phm` model-id key, so a share/reload after "Size hardware" keeps sizing the handed-off model), and every decoded `ph` value is clamped/validated the same way step 1's `pt_` keys are before it can reach `calculate()`; the math lives in `src/lib/hardwareSizing.ts`.
...
## Planner, step 1: which model for this task?

`src/lib/taskPicker.ts`'s `rankModelsForTask(catalog, constraints)` ranks the bundled catalog for a task (chat, coding, reasoning, long-context, vision, tool-use, multilingual, small-edge) under: a GPU preset × count or "any" (searches the whole bundled GPU table, smallest VRAM first, for the smallest one that runs each model), the context needed, concurrent users, weight quant and KV dtype, a license filter (any / permissive-only — the permissive list is the explicit `PERMISSIVE_LICENSES` array, currently `apache-2.0` and `mit`), and whether MoE models are allowed. It's a pure, framework-free function: filter to matching/fitting-context/license/MoE models, run the existing `calculate()` for each on its hardware, then rank — models that run first, then larger total and active params, then higher decode tok/s, then newer release date. This is a heuristic over the bundled numbers, not a benchmark, and the UI says so next to the table. The top 8 rows are shown, each with a one-line reason built from its own numbers, always including which GPU (× count) it ran on since that's not obvious under "any" (e.g. "fits with 18 GB headroom · 41 tok/s · Apache-2.0 · 128K ctx · 1× RTX 4090"), a **Use** action that loads that model/hardware/quant/workload into the Calculator, and a **Size hardware** action that hands the model to step 2. A row's own `workload` (context/users, clamped to that model's `maxPositionEmbeddings` and to `MAX_USERS` — the same clamp the Calculator itself applies on load) is what **Use** passes on, so a row's numbers and what gets loaded can never disagree.

The TaskPicker's inputs are saved in the URL under `pt_`-prefixed keys (`src/lib/taskPickerUrl.ts`), separate from the Calculator's own keys and step 2's `ph_` keys, so a plan can be shared; a link with no `pt_` keys (every link written before this existed) opens with no TaskPicker state, unchanged. Every decoded field is validated or clamped before it can reach `rankModelsForTask`: `pt_gpu` must be `any` or a real `GPU_PRESETS` name, `pt_wq`/`pt_kq` must be a known quant, and `pt_gc`/`pt_ctx`/`pt_users` are clamped to the same bounds the Calculator's own inputs use (`MIN_CONTEXT`/`MAX_USERS`/`MAX_GPUS`, plus the largest context any bundled model supports for `pt_ctx`'s upper end) — a crafted or out-of-range value falls back to the default rather than reaching `calculate()` unclamped. Those bounds live in `src/lib/limits.ts` (re-exported from `src/ui/state.ts` for the Calculator's own inputs) so `src/lib` never has to import from `src/ui`.
